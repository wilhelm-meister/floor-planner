...
// Snap helpers — Shift inverts the current snap state
const isSnapActive = (): boolean => {
  const enabled = useEditor.getState().snapEnabled
  return snapOverrideRef.shiftFree ? !enabled : enabled
}
